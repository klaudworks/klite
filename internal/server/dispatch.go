...
func (s *Server) dispatch(kreq kmsg.Request) (kmsg.Response, error) {
	key := kreq.Key()
	handler := s.handlers.Get(key)
	if handler == nil {
		// No handler registered for this key.
		// This shouldn't happen because readLoop checks RequestForKey,
		// but the key might be valid in the protocol but not implemented by us.
