...
3. `br comments list <issue-id>` — read existing context and prior decisions
...
4. Read the relevant source files. Understand the current state thoroughly.
...
5. **Validate the problem.** The issue description may be wrong or outdated.
   ...
   - Stop here.
6. **Validate the value.** Even if the problem is real, is fixing it clearly
...
7. **Check scope.** If the issue requires new user-facing functionality
   ...
   - Stop here.
8. **Design a clean solution.** Focus on the approach, not the line-by-line
...
9. **Fast-track check.** If the issue is small and clear-cut — the fix is
   ...
   review issue.
10. Otherwise, write the plan as a comment:
   ```
   br comments add <issue-id> "Plan: <plan>"
   ```
11. Promote to ready-for-implementation (status goes back to open so
